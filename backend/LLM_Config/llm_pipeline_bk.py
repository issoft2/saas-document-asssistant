...
async def infer_intent_and_rewrite(
    user_message: str,
    history_turns: Optional[List[Tuple[str, str]]] = None,
) -> Tuple[IntentType, Optional[str], str, bool]:
    """
    Returns:
...
async def llm_pipeline_stream(
    ...
    intent, rewritten, domain, chart_only = await infer_intent_and_rewrite(
    ...
    retrieval = await store.query_policies(
        tenant_id=tenant_id,
        collection_name=None, # backward compatible
    ...
    try:
        ...
        raw = rerank_resp.choices[0].message.content or "[]"
        raw = raw.strip()
        try:
    ...
    try:
        ...
        critique = (critique_resp.choices[0].message.content or "").strip().upper()
        ...
        try:
            ...
            formatted_answer = formatted_resp.choices[0].message.content or raw_answer
        ...
        try:
            ...
            if ((domain == "FINANCE" and chart_intent_trigger) or intent in {
                ...
                raw_chart = chart_resp.choices[0].message.content or ""
